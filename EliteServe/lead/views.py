...
class LeadCreateView(LoginRequiredMixin, CreateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Check if team has reached the lead limit
        lead_limit = getattr(team, 'lead_limit', None)
        if lead_limit is not None and team.leads.count() >= lead_limit:
            context['lead_limit_reached'] = True
        else:
            context['lead_limit_reached'] = False
    ...
    def form_valid(self, form):
        ...
        # Check if team has reached the lead limit
        lead_limit = getattr(self.object.team, 'lead_limit', None)
        if lead_limit is not None and self.object.team.leads.count() >= lead_limit:
            messages.error(self.request, 'Your team has reached the maximum number of leads. Your owner must upgrade your plan!')
