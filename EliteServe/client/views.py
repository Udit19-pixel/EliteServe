...
def clients_export(request):
    ...
    if team is not None:
        ...
        # Handle case where team is None
        clients = []
...
def clients_add(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # Check if team has reached the client limit
            client_limit = getattr(team, 'client_limit', None)
            if client_limit is not None and team.clients.count() >= client_limit:
